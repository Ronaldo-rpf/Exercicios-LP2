...
imgRGB loadImgRGB(char fileName[], imgRGB img);
...
int main (){
    ...
    picRGB = loadImgRGB (fileName, picRGB);

    if (picRGB._img == NULL || picRGB.img == NULL){
        printf ("Erro ao tentar ler imagem binaria.\n\n");
    }
    else{
        for (int i = 0; i < picRGB.nLin; i++){
            for (int j = 0; j < picRGB.nCol; j++){
                printf ("(%03d, %03d, %03d) ", picRGB._img[i * picRGB.nCol + j].R, picRGB._img[i * picRGB.nCol + j].G, picRGB._img[i * picRGB.nCol + j].B);
            }
            printf ("\n");
        }
    }
...
imgRGB loadImgRGB(char fileName[], imgRGB img){
    ...
    arq = fopen (fileName, "rb");
    if (arq){
        if (fread(img._img, sizeof(tRGB), img.nLin*img.nCol, arq) == img.nLin*img.nCol){
            int aux = fclose(arq);
            if (aux){
                img._img = NULL;
                img.img = NULL;
