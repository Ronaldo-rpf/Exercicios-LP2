...
int main (){
    ...
    qntd = quantosPixelsAcimaDeInt_D(mat2D, nLin, nCol, 250);
...
int quantosPixelsAcimaDeInt_D(unsigned char** img, int nLin, int nCol, unsigned char inten){
    int menores, iguais;
    menores = quantosPixelsAbaixoDeInt_D(img, nLin, nCol, inten);
    iguais = quantosPixelsNaInt_D(img, nLin, nCol, inten);
    return (nLin*nCol)-(menores+iguais);
}